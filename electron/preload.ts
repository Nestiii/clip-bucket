...
contextBridge.exposeInMainWorld('api', {
    ...
    getBucket: (bucketId: string): Promise<Bucket | null> =>
        ipcRenderer.invoke(IPC_EVENTS.DATA.GET_BUCKET, bucketId),
    getBucketStats: (): Promise<BucketStats> =>
        ipcRenderer.invoke(IPC_EVENTS.DATA.GET_BUCKET_STATS),
    getBucketName: (bucketId: string): Promise<string | null> =>
        ipcRenderer.invoke(IPC_EVENTS.DATA.GET_BUCKET_NAME, bucketId),

    // Bucket operations
    createBucket: (name: string): Promise<Bucket | null> =>
        ipcRenderer.invoke(IPC_EVENTS.BUCKET.CREATE_BUCKET, name),
    ...
    deleteBucket: (bucketId: string): Promise<boolean> =>
        ipcRenderer.invoke(IPC_EVENTS.BUCKET.DELETE_BUCKET, bucketId),
    ...
    searchBuckets: (query: string): Promise<Bucket[]> =>
        ipcRenderer.invoke(IPC_EVENTS.SEARCH.SEARCH_BUCKETS, query),
    ...
    setClipboard: (text: string): Promise<boolean> =>
        ipcRenderer.invoke(IPC_EVENTS.CLIPBOARD.SET_CLIPBOARD, text),
    clearClipboard: (): Promise<boolean> =>
        ipcRenderer.invoke(IPC_EVENTS.CLIPBOARD.CLEAR_CLIPBOARD),
    ...
    updateShortcuts: (shortcuts: Record<string, string>) =>
        ipcRenderer.invoke(IPC_EVENTS.SETTINGS.UPDATE_SHORTCUTS, shortcuts),
