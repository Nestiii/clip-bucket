...
export const setupClipHandlers = (): void => {
    ipcMain.handle(IPC_EVENTS.CLIP.SAVE_TO_BUCKET, async (_event, bucketId: string, label?: string) => {
        try {
    ...
    ipcMain.handle(IPC_EVENTS.CLIP.ADD_ITEM_TO_BUCKET, async (_event, bucketId: string, content: string, label?: string) => {
        try {
    ...
    ipcMain.handle(IPC_EVENTS.CLIP.UPDATE_CLIP, async (_event, bucketId: string, itemId: string, updates: { content?: string; label?: string }) => {
        try {
    ...
    ipcMain.handle(IPC_EVENTS.CLIP.COPY_ITEM, async (_event, text: string, hideAfterCopy: boolean = true) => {
        try {
    ...
    ipcMain.handle(IPC_EVENTS.CLIP.DELETE_CLIP, async (_event, bucketId: string, itemId: string) => {
        try {
